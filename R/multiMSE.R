...
multiMSE <- function(MOM, MPs=list(c("AvC","DCAC"),c("FMSYref","curE")),
  ...
  for(p in 1:np){
    ...
    V <- nlz(V,c(1,3),"max")
    MSYrefs <- sapply(1:nsim, DLMtool::optMSY_eq, StockPars[[p]]$M_ageArray,
  ...
  for (mm in 1:nMP) {  # MSE Loop over methods
    ...
    NextYrN <- sapply(1:nsim, function(x)
      popdynOneMICE(np,nf,nareas, maxage, Ncur=array(N[x,,,nyears,],
                                                     c(np,maxage,nareas)),
  ...
  # Store MSE info
  attr(MSEout, "version") <- paste0("MSEtool: ",
